...
public class BankTestSteps {
    ...
    public void iShouldVerifyLogoutTabDisplayed() {
        Assert.assertEquals(new AccountPageStoreTransactionDepositAndWithdraw().verifyLogOutText(), "Logout");
    }

    @And("^I click on Logout$")
    public void iClickOnLogout() {
        new AccountPageStoreTransactionDepositAndWithdraw().clickOnLogOutButton();
    }
    ...
    public void iClickOnDepositTab() {
        new AccountPageStoreTransactionDepositAndWithdraw().clickOnDepositButton();
    }

    @Then("^I Enter amount (\\d+)$")
    public void iEnterAmount(int arg0) {
        new AccountPageStoreTransactionDepositAndWithdraw().enterAmountToDeposit();
    }

    @And("^I click on Deposit Button$")
    public void iClickOnDepositButton() {
        new AccountPageStoreTransactionDepositAndWithdraw().clickOnDeposit();
    }

    @Then("^I should verify message Deposit Successful$")
    public void iShouldVerifyMessageDepositSuccessful() {
        Assert.assertEquals(new AccountPageStoreTransactionDepositAndWithdraw().verifyTextDepositSuccessful(), "Deposit Successful");
    }

    @And("^click on Withdrawal Tab$")
    public void clickOnWithdrawalTab() {
        new AccountPageStoreTransactionDepositAndWithdraw().clickOnWithDrawlButton();
    }

    @Then("^I Enter amount now (\\d+)$")
    public void iEnterAmountNow(int arg0) {
        new AccountPageStoreTransactionDepositAndWithdraw().enterAmountToWithDrawl();
    }

    @And("^I click on Withdrawal Button$")
    public void iClickOnWithdrawalButton() {
        new AccountPageStoreTransactionDepositAndWithdraw().clickOnWithDrawl();
    }

    @Then("^I should verify message Transaction Successful$")
    public void iShouldVerifyMessageTransactionSuccessful() {
        Assert.assertEquals(new AccountPageStoreTransactionDepositAndWithdraw().verifyTransactionSuccessfulText(), "Transaction successful");
    }
